...
public interface TemperatureRepository extends JpaRepository<Temperature, Integer> {
    ...
    Integer countTemperaturesBySondeAndDatePeriod(@Param("sonde") String sonde, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    //@Query("SELECT s FROM Temperature s WHERE s.sonde = :sonde AND s.date BETWEEN :startDate AND :endDate ORDER BY s.date ASC")
    @Query("SELECT new com.zohra.microservice_temperature.projection.DateProjection(s.date) FROM Temperature s WHERE s.sonde = :sonde AND s.date BETWEEN :startDate AND :endDate")
